...
function navbar(){
   require(getPath().'/view_page/navbar.php');
        }
function notifications(){
    require(getPath().'/view_page/notifications.php');
}
function login(){
   require(getPath().'/view_page/login.php');
        }
function wall(){
    require(getPath().'/view_page/wall.php');
}
function footer(){
    require(getPath().'/view_page/footer.php');
}
function take_picture(){
    require(getPath().'/view_page/take_picture.php');
}

// -- Delete image
if (isset($_GET['deleteImg']) && isset($_SESSION['username']) && isset($_POST['img_id']))
    deleteImg(strip_tags($_SESSION['username']), strip_tags($_POST['img_id']));
...
if (isset($_GET['upload']) && isset($_SESSION['username']) && isset($_POST['data']) && isset($_POST['filename'])) {
    ...
    echo "<list>";
    $target_file = Up_files(strip_tags($_POST['data']));
    if (!$target_file)
        echo "<item src=\"ERROR\"/>";
    else
...
if (isset($_GET['load_wall_images']) && isset($_POST['last_img_id']))
{
    ...
    echo "<?xml version=\"1.0\" encoding=\"utf-8\"?>";
    echo "<list>";
    $results = load_img(strip_tags($_POST['last_img_id']));
...
if (isset($_GET['load_comments']) && isset($_SESSION['username']) && isset($_POST['img_id']))
{
    ...
    echo "<?xml version=\"1.0\" encoding=\"utf-8\"?>";
    echo "<list>";
    $results = comment_loader(strip_tags($_POST['img_id']));
    foreach ($results as $values) {
...
if (isset($_GET["create"]) && isset($_POST["submit"]) && isset($_POST["username"]) && isset($_POST["pwd_1"]) && isset($_POST["pwd_2"])
    && isset($_POST["mail_1"]))
{
    valid_check(strip_tags($_POST['username']), strip_tags($_POST['pwd_1']), strip_tags($_POST['pwd_2']), strip_tags($_POST['mail_1']));
...
if (isset($_GET['username']) && isset($_GET['valid_key']))
    get_mail_conf(strip_tags($_GET['username']), strip_tags($_GET['valid_key']));
...
// -- change password
 ...
 {
     valid_check('', strip_tags($_POST['new_pwd_1']), strip_tags($_POST['new_pwd_2']), '');
     if (!isset($_SESSION['password_not_strong']))
         change_pass(strip_tags($_SESSION['username']), strip_tags($_POST['pwd_old']), strip_tags($_POST['new_pwd_1']));
    else {
 ...
 {
    if(isset($_POST["new_username"]) && isset($_POST["conf_new_username"]) && $_POST["new_username"] === $_POST["conf_new_username"] && strlen(trim($_POST['conf_new_username'])) > 0)
...
if (isset($_GET["Submit_email"]) && isset($_POST["submit"]) && isset($_POST["old_mail"]) && isset($_POST["new_mail"]) && isset($_POST["conf_new_mail"]))
    {
      if($_POST["new_mail"] === $_POST["conf_new_mail"] && strlen(trim($_POST['new_nail'])) > 0)
        change_mail(strip_tags($_SESSION["username"]), strip_tags($_POST["old_mail"]), strip_tags($_POST["new_mail"]));
        else
            header('location: ./index.php?profil');
    }
?>
