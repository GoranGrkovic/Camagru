...
function read_comment(data, img_id) {
    ...
    let check_load_more = document.getElementsByClassName(img_id + "-comment_loader");

    if (ref_last_comment.length < comment.length)
    {
        let comment_loader = document.createElement('div');
        if (check_load_more == null || check_load_more.length < 1 )
        {
            comment_loader.innerHTML =
                '<div class="' + img_id + '-comment_loader load_more">' +
                '<p>Load more..</p>' +
                '</div>';
            insertAfter(ref_last_comment[ref_last_comment.length - 1], comment_loader);
            // Add event listener on click for load more...
            comment_loader.addEventListener('click', function(){request_comments(img_id)});
        }
    }
}