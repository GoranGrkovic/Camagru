...
function deleteImg($username, $img_id)
{
    $bdd = dbConnect();
    try {
...
// comment_loader
function comment_loader($img_id){
    sleep(1);
    $bdd = dbConnect();
    try
    {
...
function new_comment($username, $img_id, $comment){
    ...
    if ($results['new_comment'] == 1) {
        ...
        $entete = "From: camagru.planethoster.world";
...
function valid_check($username, $password, $password_confirm, $mail){
    $bdd = dbConnect();
...
function submit_image($username, $data, $filter){
    ...
    if (($type == "jpeg" || $type == "jpg" || $type == "png") && check_base64_image($base64, $type)) {
        ...
        $img_link = "/application/images/wall/" . date('Y-m-d-m-s') . '.png';
        $img_date = date('Y-m-d');
        file_put_contents(getPath().'/application/images/wall/' . date('Y-m-d-m-s') . '.png', $data);
        try
        {
...
function Up_files($data){
    ...
    if ($type == "jpeg" || $type == "jpg" || $type == "png" && check_base64_image($base64, $type)) {
        ...
        $data = base64_decode(preg_replace('#^data:image/\w+;base64,#i', '', $data));
        $target_file = '/application/images/uploads/' . date('Y-m-d-m-s') . '.' . $type;
...
function userCreate($username, $password, $mail){
    ...
    $message = 'Bienvenue sur Camagru !
		...
		https://camagru.planethoster.world/controller/controller.php?username='.urlencode($username).'&valid_key='.urlencode($valid_key).'
...
function get_mail_conf($username, $valid_key){
    $bdd = dbConnect();
...
// change old password
function change_pass($username, $password, $new_password){
    $bdd = dbConnect();
    session_start();
    $bdd_password = NULL;
...
function mail_password($mail, $username){
    ...
    try
    {
        ...
        if ($request->execute(array(':mail' => $mail)) && $row = $request->fetch())
        {
		    ...
		    https://camagru.planethoster.world/index.php?login&mail_password_confirmation&username='.urlencode($username).'&password_key='.urlencode($password_key).'
...
function change_password_by_forgot($username, $password_key, $password, $password_confirm){
    ...
    if (strcmp($password, $password_confirm) != 0){
        ...
        header("Location: https://camagru.planethoster.world/index.php?login&mail_password_confirmation&username=$username&password_key=$password_key");
        exit();
    }
    try
    {
...
// change old mail
function change_mail($username, $old_mail, $new_mail){
    $bdd = dbConnect();
    session_start();
    $bdd_mail = NULL;
    $request = $bdd->prepare("SELECT * FROM users");
